...
2. Register the client's intake: business type, target public function,
   prior filing history in Thailand if any.
3. Run the advisor in read-only mode against e-GP (Electronic Government Procurement), Thailand's system managed by the Comptroller General's Department (CGD), Ministry of Finance.
...
- named referral relationship with Thai-licensed counsel or a
...
a working referral relationship with Thai-licensed counsel or a
registered agent for whatever licensed representation the law of
Thailand requires for actual public-procurement filings.
