# Business Model: Independent Public-Sector Market-Entry & Procurement Compliance Service — Thailand

## Classification

- Repository: `cloud-itonami-iso3166-tha`
- ISO 3166: `THA` (Thailand)
...
- Social impact: [:thai-manufactured-preference-access :public-spend-transparency :cross-border-friction-reduction]

## Customer

- an already-incorporated `cloud-itonami-cofog-{code}` /
  `cloud-itonami-isco-{code}` / `cloud-itonami-unspsc-{segment}` /
  `cloud-itonami-{ISIC}` operator wanting to bid on a Thai
  public contract
...
- registration walkthrough for e-GP (Electronic Government Procurement), Thailand's system managed by the Comptroller General's Department (CGD), Ministry of Finance
- business/tax registration checklist: a business registration with the Department of Business Development (DBD); VAT registration with the Revenue Department where the taxable-turnover threshold is met
- local-content / preferential-procurement navigation: no blanket local-content quota, but certain CGD tenders apply Thai-manufactured-goods preference margins
...
- this service does **not** provide legal or tax advice; characterization
  ...
  routes to Thai-licensed counsel or a registered agent
...
- **`cloud-itonami-cofog-{code}`**: a jurisdiction-agnostic operator
  ...
  blueprint AND this one to operate in Thailand).
