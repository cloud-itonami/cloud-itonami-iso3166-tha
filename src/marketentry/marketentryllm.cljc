(ns marketentry.marketentryllm
  ...
  the THA public-sector market-entry compliance actor.
...
(defn- propose-submit
  ...
  (let [e (store/engagement db subject)]
    {:summary    (str subject " 向けポータル提出提案"
                      (when e (str " (operator=" (:operator e) ")")))
     :rationale  (if e
                   (str "has-th-entity?=" (:has-th-entity? e)
                        " dbd-verified?=" (:dbd-verified? e)
     ...
     :confidence (if (and e
                          (or (not (:requires-th-entity? e))
                              (:has-th-entity? e))
                          (or (not (:requires-dbd? e))
                              (:dbd-verified? e)))
                   0.9 0.3)}))
